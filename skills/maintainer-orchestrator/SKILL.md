...
description: "Orchestrate repository work with owned lanes, preservation, proof gates, CI, decisions, and releases."
---
...
- When the user requests continuous orchestration, maintain one root-owned heartbeat and never create duplicates.
- On each wake, read the newest worker instructions and external state before steering. Continue through CI and release gates; surface only prepared owner decisions.
- Stop monitoring only when the user asks or the monitored portfolio is genuinely complete.

## Repository Scope

- Default to the current repository. Broaden to other IYENTeam repositories only when the user asks for organization-wide or multi-repository maintenance.
- Include another owner only when the user names it or the current checkout belongs to it.
- Exclude archived repositories and inactive forks unless the user asks for archaeology or a named item lives there.
- Repository-local `AGENTS.md`, `VISION.md`, release policy, and maintainer runbooks remain authoritative.
...
5. Use bounded `gh --json` searches for the broad queue map. Filter to eligible, non-archived, non-fork repositories.
...
1. Map each repository's open issues, open PRs, CI, latest release, package metadata, and unreleased changelog. Read `VISION.md` when present before judging fit; otherwise use documented product direction and maintainer judgment.
...
4. Respect the configured concurrency limit across distinct repositories. After reservation and repository-state checks, refill useful capacity from the smallest eligible queue whenever a lane completes or becomes durably blocked.
...
- This root orchestrator owns `./.maintainer-orchestrator.md`; workers do not edit it.
...
1. Check only the exact expected environment variable; never dump the environment.
2. Read the service-specific authentication instructions and use the approved secret manager when configured.
3. Prefer scoped, short-lived credentials and `op run` or equivalent injection over printing values.
4. Ask the owner only after the targeted credential path is absent, inaccessible, or requires interactive approval.

Keep credential discovery inside the worker that needs it. Report only whether access exists and the exact missing approval; never send credentials between threads.
