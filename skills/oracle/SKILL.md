...
- If the exported `OPENAI_API_KEY` is invalid and the user wants another key, use the configured secret manager. Inject the exact secret only into the single Oracle command and never print it.
- For debugging Oracle itself, prefer the local checkout after pulling `<ORACLE_REPO_PATH>`:
  - `pnpm -C <ORACLE_REPO_PATH> run build`
  - `node <ORACLE_REPO_PATH>/dist/scripts/run-cli.js ...`
