...
For PRs, collect public author context by default unless the author owns the repository or is an obvious project bot. If a `github-author-context` skill is installed, use it and include a short `Author context:` block near the top of the review. Otherwise use `gh api users/<login>` plus repository-local PR/issue history. Do not require private contributor notes.
After merge/rejection/close/review, update contributor notes only when the repository already has an established notes workflow and the interaction creates durable future-review signal.
